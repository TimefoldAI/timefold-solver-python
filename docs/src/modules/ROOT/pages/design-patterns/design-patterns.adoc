...
.. Make sure that when all planning variables have a value of `None`,
the planning entity instance is describable to the business people.
Planning variables have a value of `None` when the planning solution is uninitialized.
...
* ``str`` with or without ``datetime.time`` if no date is involved.
* ``datetime.date`` if no time is involved.
* ``datetime.datetime`` if your model only works in a single timezone without DST (Daylight Saving Time).
