...
* An instance of a type that implements `\__hash__` and `\__eq__`. See the documentation for the https://docs.python.org/3/reference/datamodel.html#object.__hash__[object class] for details.
** It's recommended to use the type `int`, `str` or `uuid.UUID`.
...
A planning entity is a Python object that changes during solving, for example a `Lesson` that changes timeslots.
...
A planning entity class needs to be annotated with the `@planning_entity` annotation.
...
[NOTE]
====
Planning entity `\__hash__` implementations must remain constant.
Therefore entity `\__hash__` must not depend on any planning variables.
Pay special attention when using data structures with auto-generated `\__hash__` as entities.
====
...
- No <<planningVariable,genuine planning variable>> is set to `None`.
- Genuine planning variable is set to `None`, but it <<planningVariableAllowingUnassigned,allows unassigned values>>.
...
A planning variable is an attribute on a planning entity.
...
The optional `value_range_provider_refs` property defines what are the possible planning values for this planning variable.
...
[NOTE]
====
A PlanningVariable annotation needs to be on a member in a class with a @planning_entity annotation.
...
By default, an initialized planning variable cannot be ``None``,
so an initialized solution will never use `None` for any of its planning variables.
...
To allow an initialized planning variable to be ``None``, set `allowsUnassigned` to ``true``:
...
Timefold Solver will automatically add the value `None` to the value range.
There is no need to add `None` in a collection provided by a ``ValueRangeProvider``.

[IMPORTANT]
====
Using a planning variable with unassigned values implies
that your score calculation is responsible for punishing (or even rewarding) variables with a `None` value.

Failure to penalize unassigned variables can cause a solution with *all* variables unassigned to be the best solution.
See the xref:responding-to-change/responding-to-change.adoc#overconstrainedPlanningWithNoneValues[overconstrained planning with `None` variable values] section in the docs for more infomation.
====
...
That class must be defined in the solver configuration and have a `@planning_entity` annotation.**
...
unless one side points to `None` and the other side does not exist.
...
[NOTE]
====
The shadow property, which is ``Collection`` (usually `List`, `Set` or `SortedSet`), can never be ``None``.
...
in a class with a `@planning_solution` or a `@planning_entity` annotation.
...
The `@planning_solution` implementation has a method that returns a `Collection` (or a ``CountableValueRange``).
...
[IMPORTANT]
====
That `Collection` (or ``CountableValueRange``) must not contain the value ``None``,
...
- Annotate the class with `@planning_entity` to make it a shadow planning entity.
...
`Integer` is required instead of `int`, as the index may be `None` if the element is not yet assigned to the list variable.
...
NOTE: The previous and next element shadow variables may be `None` even in a fully initialized solution.
The first element's previous shadow variable is `None` and the last element's next shadow variable is `None`.
...
@dataclass
class Line:
    operator: Annotated[Operator | None, PlanningVariable]
    jobs: Annotated[list[Job], PlanningListVariable] = field(default_factory=list)
    ...
----
====
...
It is annotated with a `@planning_solution` annotation.
...
In school timetabling, that `Timetable` class has the `@planning_solution` annotation,
yet every `Lesson` in an unsolved `Timetable` class is not yet assigned to a `Timeslot` (their `timeslot` property is ``None``).
...
[NOTE]
====
A `PlanningEntityCollectionProperty` annotation needs to be on a member in a class with a `@planning_solution` annotation.
It is ignored on parent classes or subclasses without that annotation.
====
...
A `@planning_solution` class requires a score property (or field), which is annotated with a `PlanningScore` annotation.
...
[NOTE]
====
A ProblemFactCollectionProperty annotation needs to be on a member in a class with a @planning_solution annotation.
It is ignored on parent classes or subclasses without that annotation.
====
...
mark its class with a `@deep_planning_clone` annotation:
...
any element thereof that is an instance of a class that has a `@deep_planning_clone` annotation.
...
Create a `@planning_solution` instance to represent your planning problem's dataset, so it can be set on the `Solver` as the planning problem to solve.

In school timetabling, a `Timetable` instance is created with the required `Room` and `Timeslot` instances
and every `Lesson` has its planning variables set to `None`:
