...
A `Solver` should only be accessed from a single thread, except for the methods that are specifically documented in docstring as being thread-safe.
...
Later calls to `get_solver_status(problem_id)` or `terminate_early(problem_id)` use that problem ID
to distinguish between the planning problems.
The problem ID must be an immutable class, such as `int`, `str` or `uuid.UUID`.

The `SolverManagerConfig` class has a `parallel_solver_count` property,
...
By default, `parallel_solver_count` is set to `AUTO`, which resolves to half the CPU cores.

To retrieve the best solution, after solving terminates normally, use `SolverJob.get_final_best_solution()`:
...
and does not want to wait any longer for a better one, call `SolverManager.terminate_early(problem_id)`.
