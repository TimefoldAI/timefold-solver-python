...
[#overconstrainedPlanningWithNoneValues]
=== Overconstrained planning with `None` variable values

If we handle overconstrained planning with `None` variable values, the overload entities will be left unassigned:
...
Use xref:constraints-and-score/score-calculation.adoc#constraintStreamsForEach[`forEachIncludingUnassigned`] and check if the xref:using-timefold-solver/modeling-planning-problems.adoc#planningVariable[planning variable] or xref:using-timefold-solver/modeling-planning-problems.adoc#bidirectionalVariable[inverse relation shadow variable] is `None`:
...
[NOTE]
====
This method is inferior to xref:overconstrainedPlanningWithNoneValues[overconstrained planning with `None` variable values].
It is likely to result in significantly worse solver performance and is also more challenging to implement.
None variable values should be preferred over virtual values if at all possible.
====
...
the lecture will not be assigned to another period or room (even if the current period and rooms fields are `None`).
...
. Any change in a `ProblemChange` must be done on the `@planning_solution` instance of ``scoreDirector.getWorkingSolution()``.
