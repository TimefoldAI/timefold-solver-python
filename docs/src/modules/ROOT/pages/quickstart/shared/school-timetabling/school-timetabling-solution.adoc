...
The `Timetable` class has an `@planning_solution` annotation,
...
** Of each `Lesson`:
*** The values of the `timeslot` and `room` fields are typically still `None`, so unassigned.
...
* The `lessons` field for which each `Lesson` instance has non-None `timeslot` and `room` fields after solving.
...
The `lessons` has an `@planning_entityCollectionProperty` annotation,
