...
A lesson's `timeslot` and `room` fields are unassigned (`None`) in the input data
and assigned (not `None`) in the output data.
...
Their containing class, `Lesson`, requires an `@planning_entity` annotation.
...
The `Lesson` class has an `@planning_entity` annotation,
