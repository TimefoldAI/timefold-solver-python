...
def _get_python_object_attribute(object_id, name):
    ...
    from ai.timefold.solver.python import PythonComparable
...
def _get_python_array_to_id_array(the_object: List):
    ...
    out = _to_java_list(list(map(lambda x: JProxy(ai.timefold.jpyinterpreter.types.wrappers.OpaquePythonReference,
...
def _set_python_object_attribute(object_id: int, name: str, value: Any) -> None:
    ...
    from ai.timefold.solver.python import PythonObject  # noqa
...
def _deep_clone_python_object(the_object: Any):
    ...
    return JProxy(ai.timefold.jpyinterpreter.types.wrappers.OpaquePythonReference, inst=the_clone, convert=True)
...
def init(*args, path: List[str] = None, include_timefold_jars: bool = True, log_level='INFO'):
    ...
    from ai.timefold.solver.python import PythonWrapperGenerator, PythonPlanningSolutionCloner, PythonList, PythonComparable  # noqa
...
def __convert_if_proxy(arg):
    from ai.timefold.solver.python import PythonComparable, PythonObject
    from ai.timefold.solver.python import PythonWrapperGenerator  # noqa
...
@JImplementationFor('ai.timefold.solver.python.PythonObject')
class _PythonObject:
...
@JImplementationFor('ai.timefold.solver.python.PythonComparable')
...
def compile_and_get_class(python_class):
    ...
    from ai.timefold.solver.python import AbstractCPythonBackedClass
